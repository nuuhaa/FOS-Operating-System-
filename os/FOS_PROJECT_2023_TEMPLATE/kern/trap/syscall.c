...
/*******************************/
void validate(uint32 va){
	//cprintf("address is %lu\n",va);
	//cprintf("User Limit is:%lu\n",USER_LIMIT);
	if(va>=USER_LIMIT || va==0||va==4018139136){
		sched_kill_env(curenv->env_id);
	}
}
void sys_free_user_mem(uint32 virtual_address, uint32 size)
{
	validate(virtual_address);
...
bool is_page_marked(struct Env* e, uint32 virtual_address){
 ...
 uint32 sys_first_fit(uint32 needed_pages){
	uint32 free_found_pages=0;
	uint32 moving_address=USER_HEAP_START+(uint32)32*1024*1024+PAGE_SIZE;

	uint32 it_num=0;
	while(moving_address<USER_HEAP_MAX){
		bool ret=is_page_marked(curenv,moving_address);
		//cprintf("marked :%d  iteraion : %lu\n",ret,it_num+1);
		if(ret==1)
			free_found_pages=0;
		else
			free_found_pages+=1;
		if(free_found_pages==needed_pages){
...
uint32 remove_blocks_after_segment_break_user(uint32 updated_break){

	uint32 removed_blocks=0;
	struct BlockMetaData *block;
	LIST_FOREACH(block,&user_block_list){
	...
	LIST_FOREACH(temp_block,&user_block_list){
		...
		if(address_plus_offset>updated_break && address<updated_break){
			...
			if(residual_size>=sizeOfMetaData())
				...
				LIST_REMOVE(&user_block_list,temp_block);
				removed_blocks+=1;
			}
		}
	}
...
void sys_env_set_nice(struct Env* env,int nice_value){
	env_set_nice(env,nice_value);
}
...
void* sys_sbrk(int increment)
{
	...
	if (increment==0)
		...
		uint32 ali_break=ROUNDUP(env->segment_break,PAGE_SIZE);
		uint32 new_brk=env->segment_break+increment;
		new_brk=ROUNDUP(new_brk,PAGE_SIZE);
		if(new_brk>env->hard_limit)
			return (void*)-1;
		sys_allocate_user_mem(ali_break,new_brk-ali_break);
		uint32 old_brk=env->segment_break;
		env->segment_break=new_brk;
		return (void*)old_brk;

	}
	else if (increment<0){
		uint32 abs_increment=-1*increment;
		uint32 aligned_old_brk=ROUNDUP(env->segment_break,PAGE_SIZE);
		uint32 new_brk=env->segment_break-abs_increment;
		if(new_brk<env->block_alloc_start)
			new_brk=env->block_alloc_start;
		uint32 aligend_new_brk=ROUNDUP(new_brk,PAGE_SIZE);
		remove_blocks_after_segment_break_user(new_brk);
		free_user_mem(env,aligend_new_brk,aligned_old_brk-aligend_new_brk);
		env->segment_break=new_brk;
		return (void*)new_brk;
	}
return NULL;
