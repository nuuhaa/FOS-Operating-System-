...
int process_command(int number_of_arguments, char** arguments)
{
	...
	for(int comm_indx = 0 ; comm_indx < NUM_OF_COMMANDS; comm_indx++){

		//command found
		if(!strcmp(commands[comm_indx].name , arguments[0])){

			if( commands[comm_indx].num_of_args== -1 && number_of_arguments >= 2)
				return comm_indx;

			//2nd status
			//command found with invalid number of arguments
			if(commands[comm_indx].num_of_args + 1 != number_of_arguments ){

				//pushing command into found commands list
				LIST_INIT(&foundCommands);
				LIST_INSERT_HEAD(&foundCommands, &commands[comm_indx]);
			...
			//4th status
			//command found with correct number of arguments
			//returning the index of the found command
			return comm_indx;
		}
	}
	...
	//3rd status
	//checking if the string is a subsequence of any command
	int char_Count;
	LIST_INIT(&foundCommands);
	for(int comm_indx = 0 ; comm_indx < NUM_OF_COMMANDS; comm_indx++){
		char_Count =0;

		for(int str_indx = 0 ; str_indx < strlen(arguments[0]); str_indx++){
			//checking if each char in the argument is a subsequence of the command or not
			if(*strfind(commands[comm_indx].name, arguments[0][str_indx]) == arguments[0][str_indx]){
				char_Count++;
			}
		}

		//checking if the matched chars == number of argument's chars
		if(char_Count == strlen(arguments[0])){
			if(LIST_SIZE(&foundCommands)==0)
				LIST_INSERT_HEAD(&foundCommands, &commands[comm_indx]);
			else
				LIST_INSERT_TAIL(&foundCommands, &commands[comm_indx]);
		}
	}

	//checking if any commands matches with the argument
	if(LIST_SIZE(&foundCommands)>0)
		return -1;
	...
	//1st status :"Invalid Command"
	//command not found and its chars aren't subsequence-matched with one or more command
	return -3;
