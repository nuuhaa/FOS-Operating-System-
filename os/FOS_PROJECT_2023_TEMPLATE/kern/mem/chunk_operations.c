...
//=====================================
void mark_page(struct Env *e,uint32 virtual_address){
	uint32*page_table;
	int ret=get_page_table(e->env_page_directory,virtual_address,&page_table);
...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 moving_address=virtual_address;
	uint32 pages=needed_pages_for_size(size);
	while(moving_address<virtual_address+PAGE_SIZE*pages){
		mark_page(e,moving_address);
		moving_address+=PAGE_SIZE;
	}
	//cprintf("Mark Limit : %x\n",moving_address);
}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 pages=needed_pages_for_size(size);
	uint32 it_num=0;
		while(moving_address<virtual_address+size){
			pt_set_page_permissions(e->env_page_directory,moving_address,0,PERM_AVAILABLE); //Unmark page
			unmap_frame(e->env_page_directory,moving_address);								//free from memory
			pf_remove_env_page(e, moving_address);											//remove from page file
			env_page_ws_invalidate(e, moving_address);										//remove from WS
		...
		if(isPageReplacmentAlgorithmFIFO()&&e->page_last_WS_element!=NULL)
		{
			...
			LIST_FOREACH(element,&e->page_WS_list)
			{
				...
				{
					struct WorkingSetElement *temp = element;
